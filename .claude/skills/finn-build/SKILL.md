...
Using the Linear connector, list issues on team `JOE` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `JOE-NNN-short-slug`, using the issue's real identifier.
...
- `Closes JOE-NNN`, using the real Linear issue identifier
