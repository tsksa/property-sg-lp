...
for (const file of pages) {
  ...
  // ── Condo district pages (generated by generate-condo-pages.mjs): same
  // structured-data and conversion invariants as the estate cluster. ──
  if (/^condo-prices\/[^/]+\/index\.html$/.test(file) || file === 'condo-prices/index.html') {
    const blocks = [...s.matchAll(/<script type="application\/ld\+json">\n([\s\S]*?)\n<\/script>/g)]
      .map((m) => { try { return JSON.parse(m[1]); } catch { return null; } });
    const nodes = blocks.flatMap((b) => (b && Array.isArray(b['@graph']) ? b['@graph'] : b ? [b] : []));
    const faq = nodes.find((n) => n && n['@type'] === 'FAQPage');
    if (!faq) fail(file, 'condo page missing FAQPage JSON-LD');
    else if ((s.match(/<summary>/g) || []).length < faq.mainEntity.length) {
      fail(file, 'condo page FAQPage has more questions than are visibly rendered');
    }
    if (file !== 'condo-prices/index.html') {
      if (!nodes.find((n) => n && n['@type'] === 'Dataset')) fail(file, 'condo district page missing Dataset JSON-LD');
      if (!s.includes('href="/condo-prices/"')) fail(file, 'condo district page missing a link back to the /condo-prices/ hub');
    }
    if (!s.includes('wa.me/')) fail(file, 'condo page has no WhatsApp CTA');
    if (!s.includes('href="/valuation.html"')) fail(file, 'condo page missing the valuation CTA');
  }
